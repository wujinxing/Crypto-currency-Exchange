...
class IssueController extends HomeController
{
	...
	public function upbuy($id, $num, $paypassword)
	{
		...
		$mo->execute('lock tables movesay_invit write ,  movesay_user_coin write  , movesay_issue write  , movesay_issue_log  write ');
		$rs = array();
		...
		$rs[] = $mo->table('movesay_issue_log')->add(array('userid' => userid(), 'coinname' => $Issue['coinname'], 'buycoin' => $Issue['buycoin'], 'name' => $Issue['name'], 'price' => $Issue['price'], 'num' => $num, 'mum' => $mum, 'ci' => $Issue['ci'], 'jian' => $Issue['jian'], 'unlock' => 1, 'addtime' => time(), 'endtime' => time(), 'status' => $Issue['ci'] == 1 ? 1 : 0));
	...
	public function unlock($id)
	{
		...
		if (time() < $tm) {
			$this->error('解冻时间还没有到<br>请在【' . addtime($tm) . '】之后再次操作');
		}
