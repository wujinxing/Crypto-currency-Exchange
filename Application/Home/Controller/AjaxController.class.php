...
class AjaxController extends HomeController
{
	...
	public function allfinance($ajax = 'json')
	{
		...
		foreach (C('coin') as $k => $v) {
			if ($v['name'] == 'cny') {
				...
				$cny['zj'] = $cny['zj'] + (round(($UserCoin[$v['name']] + $UserCoin[$v['name'] . 'd']) * $jia, 6) * 1);
			}
		}
	...
	public function getEntrustAndUsercoin($market = NULL, $ajax = 'json')
	{
		...
		if ($userCoin) {
			list($xnb) = explode('_', $market);
			list(, $rmb) = explode('_', $market);
			$data['usercoin']['xnb'] = NumToStr(floatval($userCoin[$xnb]), 8) * 1;
			$data['usercoin']['xnbd'] = NumToStr(floatval($userCoin[$xnb . 'd']), 8) * 1;
			$data['usercoin']['cny'] = NumToStr(floatval($userCoin[$rmb]), 8) * 1;
			$data['usercoin']['cnyd'] = NumToStr(floatval($userCoin[$rmb . 'd']), 8) * 1;
		}
		else {
			$data['usercoin'] = null;
		}
	...
	public function getChat($ajax = 'json')
	{
		...
		if (!$chat) {
			$chat = M('Chat')->where(array('status' => 1))->order('id asc')->limit(500)->select();
			S('getChat', $chat);
		}

		if ($chat) {
			foreach ($chat as $k => $v) {
				$data[$k] = array((int) $v['id'], $v['username'], $v['content']);
			}
		}
		else {
			$data = '';
