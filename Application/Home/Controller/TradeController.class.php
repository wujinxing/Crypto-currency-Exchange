...

{
    ...
    public function deepths($market = NULL)
    ...
    public function deepth($market = NULL)
...

    {
        ...
        if ($type == 1) {
            ...
            if ($user_coin[$xnb] < $num) {
                ...
                $this->error(C('coin')[$xnb]['title'] . '余额不足了！');
...

    {
        ...
        for ($new_trade_movesay = 0; ; ) {
            ...
            if ($buy && $sell && (0 <= floatval($buy['price']) - floatval($sell['price']))) {
                ...
                $mo->execute('lock tables movesay_trade write ,movesay_trade_log write ,movesay_user write,movesay_user_coin write,movesay_invit write');
                ...
                $rs[] = $mo->table('movesay_trade_log')->add(array('userid' => $buy['userid'], 'peerid' => $sell['userid'], 'market' => $market, 'price' => $price, 'num' => $amount, 'mum' => $mum, 'type' => $type, 'fee_buy' => $buy_fee, 'fee_sell' => $sell_fee, 'addtime' => time(), 'status' => 1));
                ...
                if ($price < $buy['price']) {
                    ...
                    if ($chajia) {
                        ...
                        if ($chajia <= round($chajia_user_buy[$rmb . 'd'], 8)) {
                            ...
                            \Think\Log::write('错误91交易市场' . $market . '出错：买入订单:' . $buy['id'] . '卖出订单：' . $sell['id'] . '交易方式：' . $type . '成交数量' . $amount, '成交价格' . $price . '成交总额' . $mum . "\n");


                            \Think\Log::write('交易市场' . $market . '出错：买入订单:' . $buy['id'] . '卖出订单：' . $sell['id'] . '成交数量' . $amount . '交易方式：' . $type . '卖家更新冻结虚拟币出现误差,应该更新' . $chajia . '账号余额' . $chajia_user_buy[$rmb . 'd'] . '实际更新' . $chajia_save_buy_rmb);
                            ...
                            \Think\Log::write('错误92交易市场' . $market . '出错：买入订单:' . $buy['id'] . '卖出订单：' . $sell['id'] . '交易方式：' . $type . '成交数量' . $amount, '成交价格' . $price . '成交总额' . $mum . "\n");


                            \Think\Log::write('交易市场' . $market . '出错：买入订单:' . $buy['id'] . '卖出订单：' . $sell['id'] . '成交数量' . $amount . '交易方式：' . $type . '卖家更新冻结虚拟币出现错误,应该更新' . $chajia . '账号余额' . $chajia_user_buy[$rmb . 'd'] . '进行错误处理');
...

    {
        ...
        $trade = M('Trade')->where(array('id' => $id, 'userid' => userid(), 'status' => 0))->find();
