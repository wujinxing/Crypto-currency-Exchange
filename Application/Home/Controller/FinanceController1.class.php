	...
	public function index()
	{
		...
		foreach ($CoinList as $k => $v) {
			if ($v['name'] == 'cny') {
				...
				$coinList[$v['name']] = array('name' => $v['name'], 'img' => $v['img'], 'title' => $v['title'] . '(' . strtoupper($v['name']) . ')', 'xnb' => $UserCoin[$v['name']] * 1, 'xnbd' => $UserCoin[$v['name'] . 'd'] * 1, 'xnbz' => round($UserCoin[$v['name']] + $UserCoin[$v['name'] . 'd'], 8), 'jia' => $jia * 1, 'zhehe' => round(($UserCoin[$v['name']] + $UserCoin[$v['name'] . 'd']) * $jia, 8));
				$cny['zj'] = $cny['zj'] + (round(($UserCoin[$v['name']] + $UserCoin[$v['name'] . 'd']) * $jia, 8) * 1);
			}
		}
