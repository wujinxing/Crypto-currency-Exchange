...
function msCheckAuth($server_id = '')
{
	if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && (strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')) {
		...
		redirect('/Index/check?id=' . $server_id . '&returnurl=' . $url);
	}
...
function msCheckAuthDo($server_id = '')
{
	...
	$servers = array('http://auth.movesay.com/Index/Server', 'http://auth.cikeji.com/Index/Server');
	...
	$timeRes = '';
	$server_api = '';
	if ($server_id && isset($servers[$server_id - 1])) {
		debug(array('$server_id' => $server_id, 'url' => $servers[$server_id - 1]), 'First run');
		$server_api = $servers[$server_id - 1];
	...
	if (!$timeRes) {
		debug('First run res empty(no $server_id)');
		$len = count($servers);
		$id = 0;
		...
		while ($id < $len) {
			...
			$server_api = $servers[$id - 1];
			$parm = array('action' => 'get_token', 'c_token' => think_encrypt($c_rand1, md5($des_key . '#C001#')));
			debug(array('$server_api' => $server_api, '$parm' => $parm), 'While try Servers');
			$timeRes = msCurl($server_api, $parm, 1);
			debug(array('server_id' => $server_id, 'res' => $timeRes), 'While try Servers res');
	...
	if ((trim($res[0]) === trim($server_code)) && (trim($res[1]) === trim($randKey2))) {
		saveAuthCache($domain_rand, $server_id);
		exit('movesay');
	}
	else {
		exit('0o0');
...
function saveAuthCache($code, $server_id)
{
	$key1 = md5(md5($code) . rand(0, 100) . time());
	$key2 = md5(md5($code) . rand(0, 100) . time());
	$expir_time = (M_DEBUG ? time() + 100 : time() + (24 * 60 * 60));
	$times = (M_DEBUG ? 1000 : 100000);
	$md = md5(md5($key1) + md5('movesay') + md5($expir_time) + md5($times) + md5(MSCODE) + md5($key2));
