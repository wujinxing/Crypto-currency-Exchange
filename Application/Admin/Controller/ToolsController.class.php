...
class ToolsController extends AdminController
{
	...
	{
		$this->redirect('/Admin/Tools/database/type/export');
		$this->display();
	}
			...
			if ($qb_list[$id]['status']) {
				...
				if (!isset($json['version']) || !$json['version']) {
					...
					if (!$qb_list[$id]['zc_user']) {
						echo json_encode(array('status' => 1, 'info' => $qb_list[$id]['title'] . '运行正常(无手续费模式,如要开启收取手续费,请配置官方手续费地址)' . $update_str));
						exit();
					}
					else {
						$valid_res = $CoinClient->validateaddress($qb_list[$id]['zc_user']);

						if (!$valid_res['isvalid']) {
							echo json_encode(array('status' => -2, 'info' => $qb_list[$id]['title'] . '手续费地址:' . $qb_list[$id]['zc_user'] . ' 不是一个有效地址,建议:1.填写一个有效地址 2.填写地址为0 (没有转入转出手续费)' . $update_str));
							exit();
						}
					}
