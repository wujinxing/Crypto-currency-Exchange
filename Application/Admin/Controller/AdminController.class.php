...
class AdminController extends \Think\Controller
{
	...
	final protected function checkRule($rule, $type = Admin\Model\AuthRuleModel::RULE_URL, $mode = 'url')
	{
		if (IS_ROOT) {
			return true;
		}
		...
		if (empty($menus)) {
			...
			if ($current) {
				...
				foreach ($menus['main'] as $key => $item) {
					...
					if (!IS_ROOT && !$this->checkRule($item['url'], \Admin\Model\AuthRuleModel::RULE_MAIN, null)) {
					...
					if ($item['title'] == $nav_first_title) {
						...
						if (!IS_ROOT) {
							...
							foreach ($second_urls as $key => $to_check_url) {
								...
								if ($this->checkRule($rule, \Admin\Model\AuthRuleModel::RULE_URL, null)) {
